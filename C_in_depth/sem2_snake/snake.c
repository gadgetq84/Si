...
void changeDirection(snake_t* snake, const int32_t key)
{
	for (int i=0;i<3;i++)
	{
	if (key == snake->controls[i].down && checkDirection(snake,key))
		snake->direction = DOWN;
	else if (key == snake->controls[i].up && checkDirection(snake,key))
		snake->direction = UP;
	else if (key == snake->controls[i].right && checkDirection(snake,key))
		snake->direction = RIGHT;
	else if (key == snake->controls[i].left && checkDirection(snake,key))
